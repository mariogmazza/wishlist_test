...
class Catagories2Sub extends Component {
  state={seats:[]}

  componentWillMount(){
    // var fixedItem = item.replace(' ', '_');
    var searchThisCat = this.props.history.location.pathname.slice(10).replace('_', ' ')
 
     
    API.getItemsFromCat(searchThisCat).then((res) => {
  ...
  render() {
    ...
    {this.state.seats.map((item,index)=>{
      var fixedItem = item.replace(' ', '-');
        ...
        <Link to={"/category/"+ fixedItem}>
              <Card raised>
                <Image src={Sparcoseat} style={{maxWidth:120}}/>
                <Card.Content>
                  <Card.Header textAlign='center' >{item}</Card.Header>
